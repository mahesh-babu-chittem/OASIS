...
# ✅ Normalize embeddings (safe improvement)
...
for i in tqdm(range(len(image_paths)), desc="pipeline"):
    ...
    # ✅ Increased retrieval depth (better recall)
    D, I = index.search(query_dino, 50)
    candidates = I[0][1:]
    ...
    for idx in candidates:
        ...
        # ✅ Adaptive fusion (better than fixed weights)
        h_norm = (h_score + 1) / 2
        weight = 0.5 + 0.3 * ml_score
        score = weight * h_norm + (1 - weight) * ml_score
...
for query_id in sample_ids:
    ...
    for rank, (idx, score) in enumerate(results):
        print(f"{rank+1}. {image_paths[idx]} | Score: {score:.4f}")
        match_names.append(image_paths[idx])
